...
var Inflector = {
	...
	add_irregular: function(singular, plural) {
		Inflector.Inflections.irregular.push([singular, plural]);
	},

	add_uncountable: function(word) {
		Inflector.Inflections.uncountable.push(word);
	}
};
...
extend(String, {
	pluralize: function(string, count, plural) {
		if (typeof count == 'undefined') {
			...
			return 1 == parseInt(count) ? string : (plural || Inflector.pluralize(string));
		}
	},

	singularize: function(string, count, singular) {
		if (typeof count == 'undefined') {
			return Inflector.singularize(string);
		} else {
			return 1 == parseInt(count) ? (singular || Inflector.singularize(string)) : string;
		}
	},

	is_singular: function(string) {
		if(String.singularize(string) === null && string)
			return true;
		return false;
	}
});
...
extend(String.prototype, {
	pluralize: function(count, plural) {
		return String.pluralize(this, count, plural);
	},

	singularize: function(count, plural) {
		return String.singularize(this, count, plural);
	},

	is_singular: function() {
		return String.is_singular(this);
	}
});
